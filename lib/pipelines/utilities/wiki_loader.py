...
from utilities.text_manipulation import word_model
from utilities.text_manipulation import extract_sentence_words
from pathlib2 import Path
import re
from utilities import wiki_utils
import os

import utilities

# logger = utilities.setup_logger(__name__, 'train.log')
...
class WikipediaDataSet(Dataset):
    def __init__(self, root, word2vec, train=True, manifesto=False, folder=True, high_granularity=True):
    ...
    def __getitem__(self, index):
        ...
        return read_wiki_file(Path(path), self.word2vec, ignore_list=True, remove_special_tokens=True,
                              high_granularity=self.high_granularity, return_as_sentences=True)
